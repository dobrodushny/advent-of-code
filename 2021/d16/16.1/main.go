...
func calcVersions(p *Packet) int {
	var rc func(*Packet, int) int
	rc = func(p *Packet, res int) int {
		res += (*p).Version()
		v, ok := (*p).(*OperatorPacket)
		if ok {
			for _, sub := range v.SubPackets {
				res += rc(sub, 0)
			}
		}
		return res
